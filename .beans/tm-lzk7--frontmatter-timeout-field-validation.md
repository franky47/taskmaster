...
updated_at: 2026-04-07T12:28:50Z
parent: tm-7fv4
---
...
- [ ] `timeout` field added to `rawFrontmatter` Zod schema as optional string
- [ ] Valid duration strings (`"30s"`, `"5m"`, `"2h"`) are accepted and converted to milliseconds
- [ ] Invalid strings (e.g. `"abc"`, `""`) are rejected with a descriptive error
- [ ] Values under 1 second (e.g. `"500ms"`, `"0s"`) are rejected with a descriptive error
- [ ] The field threads through both agent and run variants in the schema transform
- [ ] `tm validate` catches invalid timeout values
- [ ] Tests cover valid inputs, invalid inputs, boundary cases (exactly 1s), and both variants
